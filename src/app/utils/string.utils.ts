 ...
 */
export const composeExhangeRateString = (response: BaseCurrencyRatesResponseType): CurrencyRateDisplayType[] =>
	Object.entries(response.rates).reduce<CurrencyRateDisplayType[]>(
		(acc, curr) => [...acc, { [curr[0]]: `${curr[0]}/${response.base}: ${formatRateForDisplay(curr[1])}` }],
		[]
	);
